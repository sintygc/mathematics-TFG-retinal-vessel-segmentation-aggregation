function [args, cancelled, elapsedGrey, timerElapsed] = seg_iuwt(vessel_data, args, prompt, timerElapsed)
...
if prompt %no se usa
    ...
    vessel_data.w = iuwt_vessels(im, args.iuwt_w_levels); % w is the first segmentation con vasos en negro!!
    vessel_data.grey = 1-scale_01(vessel_data.w); %vessel_data.grey: 584x565 double u=329879 [-0.1630,0.1981] % ahora con vasos en blanco!
    elapsedGrey = toc(timerElapsed);
    timerElapsed = tic;
    %binarization
    vessel_data.bw = percentage_segment(vessel_data.w, args.iuwt_w_thresh, args.iuwt_dark, vessel_data.bw_mask); % the image is logical
    ...
    vessel_data.bw = clean_segmented_image(vessel_data.bw, args.iuwt_px_remove * scale, args.iuwt_px_fill * scale); % the image is logical
end
