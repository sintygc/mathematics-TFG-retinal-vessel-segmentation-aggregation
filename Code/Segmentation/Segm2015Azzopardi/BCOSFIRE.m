function resp = BCOSFIRE(image, filter1, filter2, preprocessthresh)
...
%% Filters application
%image: 584x565x3 double u=256 [0,1]
[image mask] = preprocess(image, [], preprocessthresh); %image con vasos negros
%image: 584x565 double u=227740 [0.0109,0.9838]
%mask: 584x565 double u=2 [0,1]
image = 1 - image; %vasos blancos y máscara blanca

% Apply the symmetric B-COSFIRE to the input image
resp1 = applyCOSFIRE(image, symmfilter);
resp1 = resp1{1}; %resp1: 584x565 double u=243936 [0,0.2020]

% Apply the asymmetric B-COSFIRE to the input image
resp2 = applyCOSFIRE(image, asymmfilter);
resp2 = resp2{1}; %resp2: 584x565 double u=223018 [0,0.1786]

% Final response
resp = resp1 + resp2; %resp: 584x565 double u=252443 [0,0.3806]
%imagen segmentada en escala de grises
resp = rescaleImage(resp .* mask, 0, 255); %resp: 584x565 double u=220266 [0,255]
