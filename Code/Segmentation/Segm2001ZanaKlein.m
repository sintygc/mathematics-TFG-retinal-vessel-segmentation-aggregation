function [greyRes, elapsedGrey, bwRes, elapsedBW] = Segm2001ZanaKlein(image,k,th)
...
% lap=imgaussfilt(Ssum,7/4,"FilterSize",7);
...
resultado=Sres1; %resultado: 584x565 double u=5729 [0,8318]
% greyRes=resultado/max(max(resultado)); %result: 584x565 double u=5729 [0,1]
greyRes = 1-(resultado-min(min(resultado)))/(max(max(resultado))-min(min(resultado)));
...
% result=sum(ima,3); % result=ima
% bwRes=result/max(max(result));
bwRes = greyRes>=th;
